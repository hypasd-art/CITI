...
with open('./toolalpaca_training_chat.json', 'r', encoding='utf-8') as f:
    data_toolalpaca = json.load(f)
    for data in data_toolalpaca:
        data["task_types"] = 1

sample_list = random.sample(range(1, len(data_alpaca_gpt4)), 3000)
merged_data.extend([data_alpaca_gpt4[i] for i in sample_list])

sample_list = random.sample(range(1, len(data_metamathqa)), 3000)
merged_data.extend([data_metamathqa[i] for i in sample_list])

sample_list = random.sample(range(1, len(data_triviaqa)), 3000)
merged_data.extend([data_triviaqa[i] for i in sample_list])

sample_list = random.sample(range(1, len(data_codealpaca)), 3000)
merged_data.extend([data_codealpaca[i] for i in sample_list])

merged_data.extend(data_toolalpaca)


random.shuffle(merged_data)
with open('./toolalpaca_training_chat_merge.json', 'w', encoding='utf-8') as fout:
