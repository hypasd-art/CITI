...
def load_model_and_tokenizer(model_path, base_model, moelora_allocation):
    ...
    model = Phi3ForCausalLM_MOLoRA.from_pretrained(base_model, device_map="auto",config=config)
...
def parse_output(output, openapi_spec, function_proj,retrieval_available, base_url):
    ...
    if action == "retrievalDataFromFile":
        message = func_retrieval(action_input) # ["retrieved_info"]
...
def test_single_scene(model, tokenizer, test_list, server_url, enable_getDetails=True):
    ...
    for instruction in tqdm(test_list["Instructions"]):
        ...
        chat_prompt = chat_prompt.replace("the action to take, must be one of [getDetails, sendHttpRequest, getClientRequestData, testProxyHeaders, simulateStatusCode].\n", 
                                          prompt_action)
        input = "<|system|>\n" + default_system + "\n\n" + "API description:" + tool + chat_prompt + "<|end|>\n" + \
                "<|user|>\n" + instruction + "<|end|>\n" + \
                "<|assistant|>\n"
        ...
        try_num = 0
        while try_num < 10:
            output = generate_stream(model, tokenizer, input)
            logging.info("----------------------------------------------")
            # print("------------------------------------------------------------------------------------------------")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--moelora_allocation", type=str, default="")
    ...
    model, tokenizer = load_model_and_tokenizer(args.model_path, args.base_model, args.moelora_allocation)
