...
from .tuners import LoraModel_local, PrefixEncoder_local, PromptEmbedding_local, PromptEncoder_local
from .utils import (
...
class PeftModel_local(PushToHubMixin, torch.nn.Module):
    """
    ...
    **Attributes**:
    ...
    def __init__(self, model, peft_config: PeftConfig_local): # casualLM, LoraConfig
        super().__init__()
        self.peft_config = peft_config
        self.base_model = model
        self.config = self.base_model.config
        self.modules_to_save = None
        if isinstance(self.peft_config, PromptLearningConfig_local):
            self._setup_prompt_encoder()
        else: # --------------> here
            self.base_model = LoraModel_local(peft_config, model)
    ...
    def save_pretrained(self, save_directory, **kwargs):
        ...
        # output_state_dict = get_peft_model_state_dict_local(self, kwargs.get("state_dict", None))
    ...
    @classmethod
    def from_pretrained(cls, model, model_id, enable_blc_weight=True, **kwargs):
        r"""
        Args:
        ...
        # load the config
        config = PEFT_TYPE_TO_CONFIG_MAPPING[PeftConfig_local.from_pretrained(model_id).peft_type].from_pretrained(model_id)
        if not enable_blc_weight:
            config.blc_weight = 0.0
        ...
        model = set_peft_model_state_dict_local(model, adapters_weights)
    ...
    def _setup_prompt_encoder(self):
        ...
        if self.peft_config.num_transformer_submodules is None:
            self.peft_config.num_transformer_submodules = (
                2 if self.peft_config.task_type == TaskType_local.SEQ_2_SEQ_LM else 1
            )
        ...
        if self.peft_config.peft_type == PeftType_local.PROMPT_TUNING:
            prompt_encoder = PromptEmbedding_local(self.peft_config, self.word_embeddings)
        elif self.peft_config.peft_type == PeftType_local.P_TUNING:
            prompt_encoder = PromptEncoder_local(self.peft_config)
        elif self.peft_config.peft_type == PeftType_local.PREFIX_TUNING:
            prompt_encoder = PrefixEncoder_local(self.peft_config)
        else:
    ...
    def get_prompt_embedding_to_save(self):
        ...
        if self.peft_config.peft_type == PeftType_local.PREFIX_TUNING:
    ...
    def get_base_model(self):
        ...
        """
        return self.base_model if isinstance(self.peft_config, PromptLearningConfig_local) else self.base_model.model
...
class PeftModelForSequenceClassification_local(PeftModel_local):
    ...
    **Attributes**:
    ...
    def __init__(self, model, peft_config: PeftConfig_local):
    ...
    def forward(    # pylint: disable=W0221
        ...
        **kwargs,
        ...
        if not isinstance(self.peft_config, PromptLearningConfig_local):
...
class PeftModelForCausalLM_local(PeftModel_local):
    ...
    def prepare_inputs_for_generation(self, *args, **kwargs):
        ...
        if isinstance(self.peft_config, PromptLearningConfig_local):
            if model_kwargs["past_key_values"] is None and self.peft_config.peft_type == PeftType_local.PREFIX_TUNING:
...
class PeftModelForSeq2SeqLM_local(PeftModel_local):
    """
...
class PeftModelForTokenClassification_local(PeftModel_local):
    """
    Peft model for sequence classification tasks.
